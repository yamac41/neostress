	...
	if($_SERVER['REQUEST_METHOD'] === 'POST'){
		...
		while($ticket = $GetTickets -> fetch(PDO::FETCH_ASSOC)) {
			...
			$data[] = array(
		        ...
		        "created"=>$created,
		        "action"=>'<button type="button" id="ticket-details" style="color:#fff;font-size:.6rem;" onclick="ViewTicket('.$ticket['id'].')" class="btn btn-warning btn-sm"><i class="fa-solid fa-up-right-from-square"></i></button> <button type="button" style="color:#fff;font-size:.6rem;" id="delete-ticket" onclick="DeleteTicket('.$ticket['id'].')" class="btn btn-danger btn-sm"><i class="fa-solid fa-trash-can"></i></button>'
